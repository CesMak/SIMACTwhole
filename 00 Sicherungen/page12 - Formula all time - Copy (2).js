...
function parse(id, content) {
	...
	content = ersetze_alles_zwischen(content,"-b(",")");
	
	//content = ersetze_alles_zwischen(content,"-i(",")");
	
	//content = ersetze_alles_zwischen(content,"-g(",")");

	//content = ersetze_alles_zwischen(content,"-r(",")"); //TODO - letze Klammer zu!
...
function ersetze_alles_zwischen(content,zwischen1,zwischen2){
	...
	for (j = 0; j < result.length; j++) {
		...
		if (result[0][0].length > 1) { // das 0 0 muss hier bleiben, da pro suchlauf result um eines kleiner wird! da ja die $$ ersetzt werden!
			var dazwischen = result[0][0];
...
var getFromBetween = {
	...
	getAllResults: function (sub1, sub2) {
		...
		if (this.string.indexOf(sub1) > -1 && this.string.indexOf(sub2) > -1) {
			this.getAllResults(sub1, sub2); // wird nicht ausgeführt.
		} else
			return;
	},
