...
//	var markup = document.documentElement.innerHTML;
	...
	for(var zz=0;zz<data.length;zz++){
		datatext=datatext+"Data content1 of id"+zz+":  <br>"+data[zz]["Content1"]+"<br> <br>";
	}
	var myWindow = window.open("", "_blank");
	myWindow.document.write("<p>Parsed Text Content of all Boxes(Hauptcontainer.textContent) : <br> <br></p> <p>"+textContent+"</p>"+"<p>"+datatext+"</p>");////myWindow.document.write("<p>This is 'MsgWindow'. I am 200px wide and 100px tall!</p>");
	localStorage.clear();
}
...
function add_click(id){
	...
	if (gibts_die_Add_Box_zur_id_schon == null) {
		...
		if(id==-1){
			...
			console.log("standard add click")
			id=0;
...
function build_Head(id, ue, kat1, kat2, neuer_Head_nach_hier, name_neue_container_box) {
	...
	var fragment_h3 = create(`<h3>[<a id=` + id + ` href="javascript:add_click(` + id + `)">Add</a>,<a id=` + id + ` href="javascript:bearbeiten_click(` + id + `)">Edit</a>, <a href="` + "#divcontent" + id + `" data-toggle="collapse"> Ausblenden</a>` + ", Bewegen, Markieren, Löschen] YOUTUBE Links, WIKI Links, Querverweise?]" + "</h3>");
 ...
 */
	...
	/* console.log("save_Add wurde aufgerufen! \n mit dieser id: "+id+"\n indexx:"+indexx);
	console.log(data);// TODO hier gehts weiter.
	if(id==0){
			buildWebsiteContent(indexx,"Containerboxstandart2"); //data[indexx] wird neue aufgebaut! of new element and build it nach "Containerbox"+id 
	}
	else{
		buildWebsiteContent(indexx,"Containerbox"+id);
	} */
	...
	//TODO hier bauch ich die ganze website nochmal - einfachste lösung damit die reihenfolge passt:
	/* for(var pp=0;pp<data.length;pp++){
		buildWebsiteContent(pp);//,"Containerbox"+data[pp]["id"]);
	} */
}
